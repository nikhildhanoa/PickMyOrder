			...
			<div class="input-group md-form form-sm form-2 pl-0">
				...
				<span><i class="flaticon2-search-1 text-muted"></i></span>
					...
					<?php 
					  ...
					  if($listid==40)
					  {
						$AllCat=$this->db->query("select * from dev_product_cat,dev_products where  dev_product_cat.id=categories  group by categories");
					  }
					  else
					  {
						$AllCat=$this->db->query("select * from dev_product_cat,dev_products where  dev_product_cat.id=categories and list_id like'%".'"'.$listid.'"'."%' group by categories");
					  }
					  ...
					  if($listid==40)
					  {
						$manufatureobj=$this->db->query("select Manufacture from dev_products  group by Manufacture");
					  }
					  else
					  {
					   $manufatureobj=$this->db->query("select Manufacture from dev_products where list_id like'%".'"'.$listid.'"'."%' group by Manufacture");
					  }
			...
			<div class="d-flex align-items-center">
				...
				<select class="fillterselect form-control" id="categoryfillter" style="margin-right: 25px;">
					...
					<?php foreach( $AllCat as $singe){?>
						<option value="<?php echo $singe['categories'];?>"><?php echo $singe['cat_name'];?></option>
					<?php } ?>
				</select>
...
<hr>
  ...
  <table class="table background table-striped" id="all_list">
	...
	if($listid==40)
	{
		$SingleListObject=$this->db->query("select  distinct * from dev_products");	
	}
	else
	{
	...
	foreach($single_Product_index as $single_Product)
	{ 
...
<?php $checkcat=$this->ProductCategoryModal->GetAllCat($bid); if($checkcat){ ?>
  <ul class="nav nav-tabs" style="margin-bottom:10px">
    <li class="<?php echo (isset($tab)) ? ($tab['sub']=="pd")? 'active' : '' : 'active'; ?>"><a data-toggle="tab" href="#pd" style="padding: 10px 13px;">Product Details</a></li>
    <li class="disablealert disabled<?php //echo (isset($tab)) ? ($tab['sub']=="add_img")? 'active' : '' : ''; ?>"><a data-toggle="tab" href="#ai" style="padding: 10px 13px;" >Add Images</a></li>
    <li class="disablealert disabled<?php //echo (isset($tab)) ? ($tab['sub']=="sc")? 'active' : '' : ''; ?>"><a data-toggle="tab" href="#sc" style="padding: 10px 13px;">Suppliers Cost</a></li>
	 <li class="disablealert disabled<?php //echo (isset($tab)) ? ($tab['sub']=="sp")? 'active' : '' : ''; ?>"><a data-toggle="tab" href="#sp" style="padding: 10px 13px;">Specification</a></li>
	  <li class="disablealert disabled<?php //echo (isset($tab)) ? ($tab['sub']=="ao")? 'active' : '' : ''; ?>"><a data-toggle="tab" href="#ao" style="padding: 10px 13px;">Add Option</a></li>
	   <li class="disablealert disabled<?php //echo (isset($tab)) ? ($tab['sub']=="pm")? 'active' : '' : ''; ?>"><a data-toggle="tab" href="#pm" style="padding: 10px 13px;">PDF Manual</a></li>
	   <li class="li-file"><form id="sendcsvtoanotherfile" enctype="multipart/form-data" method="post" action="uploadcsvfilebyajax"><label style="padding: 10px 13px;">Import<input type="file" accept=".csv" id="importcsv" size="60" name="thisiscsv"></label></form><li>
	   
	     <li style="margin: 0 12px;" class="li-file"><form id="sendcsvtoanotherfilebulk" enctype="multipart/form-data" method="post" action="uploadcsvfilebyajaxbulk"><label style="padding: 10px 13px;">Import<input type="file" accept=".csv" id="importcsvbulk" size="60" name="thisiscsv"></label></form></li>
	   
	   <li class="li-file firstli" style="margin-left:10px"><span class="luckinimport btn btn-info" style="padding: 10px 12px;margin-left: -8px;border-radius: 4px;background-color: #5578eb;font-weight:700">Lukins</span><li>
  ...
  <ul style="display:flex;justify-content:flex-end;list-style:none;position: relative;left: -124px;">
  <li style="padding: 0 50px;"><p style="font-weight:700">Lukins</p></li>
  <li><p style="font-weight:700">Bulk</p></li>
  </ul>
 <!-----------------------------------------------Modal popup----------------------------------------->
...
<div class="form-group">
 ...
 <div class="form-group ">
    ...
    <input readonly type="text" class="form-control" placeholder="" value="<?= isset($_POST['Description'])? $_POST['Price_INC_VAT']: '';?>" name="Price_INC_VAT" id="pincvat">
      ...
      <label>Add To Van-Stock</label>
          <input <?php if(isset($_POST[''])){ if($_POST['']=='1'){?> checked <?php }  } ?>   type="checkbox" value="1" name="invan" class="method" id="vanstock">
	  </div>
	  <div id="isVan"  style="display:none" class="Isvan">
	  <label>Stock Level</label>
	  
	  
	  <input type="text" name="stocklevel" value="" class="form-control form-control-solid vanlevel" > 
	  <label>Reorder Level</label>
	  
	  
	  <input type="text" name="reorderlevel" value="" class="form-control form-control-solid vanlevel" >
	  <!-- <div class="form-group">
      <label>Suppliers</label>
      <select class="form-control form-control-solid"  name="supplierid" id="">
	  <option  value="" selected disabled>--Select--</option>
	  <?php foreach($supplierarray as $list){?>
	  <option  <?php if(isset($_POST[''])){ if($_POST['']==$list['']){?> selected <?php }  } ?> value=<?php echo $list['id']; ?>><?php echo $list['suppliers_name']; ?></option>
	  <?php } ?>
 </select>
	  </div>-->
	  </div>
	  ...
	  <input type="number" name="InStock" min="0" class="form-control form-control-solid" >
	   </div>
 ...
 <div class="form-group">
  ...
  ?>
   <div class="form-group">
      <label class="control-label col-sm-4" for="name">Main Category</label>
	  <div class="col-md-8">
...
$('#man').click(function(){
 ...
 var form_data = new FormData($('#product_form_image')[0]);;	 
   ...
   $('#importcsv').change(function(){
    ...
    //import csv bulk
			 $('#importcsvbulk').change(function(){
			$('#sendcsvtoanotherfilebulk').submit();
		     });
 ...
 jQuery('.fillterselect').change(function(){
	  ...
	  catid=$("#categoryfillter").val(); 
	  list_id=<?php echo $_GET['listid']; ?>;
	  manufactures=$("#manufacturefillter").val();
	   ...
	   jQuery.ajax({
		...
		method:"POST",
		data:{'cat_id': catid,'list_id':list_id,'manufactures':manufactures},
		success:function(d){
			if(d)
			{
