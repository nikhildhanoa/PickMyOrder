...
/***********************************************************/
         $in_stock=$single['in_stock'];
         $in_stock=$in_stock-$quantity;
		 
		 $procode_stocklevel=$single['stocklevel'];
		 
		 $procode_reorder_value=$procode_stocklevel-$in_stock;
		 
		 $reorder_level=$single['recordlevel'];
		 
		 $toback_instock=$single['toback_instock'];
		 
		 $this->db->query("UPDATE dev_products SET in_stock='$in_stock' WHERE id='$product_id'");
		
		 if($reorder_level >= $in_stock){
         ...
         if($toback_instock==''){	
				...
				{	$SupplierCostArray=explode('=>',$supplier_with_cost_array[$i]);
				...
				/* Insert reorder in order table -------------------------------------------------------------- */
					...
					foreach($supplier_array as $key=>$value)
					{
						...
						
						{
							...
							$single_order=array('vat_class'=>$vat_class,'total_ex_vat'=>$single_vat,'ex_vat'=>$ex_vat,'qty'=>$qty,'date'=>$date,'ProductCode'=>$ProductCode,'reffrence_id'=>$refrence_id,'product_id'=>$product_id,'image'=>$image);
						...
						$order=array('business_id'=>$Busines_id,'total_ex_vat'=>$total_price_ex_vat,'date'=>$date,'po_reffrence'=>$po_reffrence,'total_inc_vat'=>$inclusive_vat,'status'=>0,'odrdescrp'=>"Van Stock Re order",'givenprojectid'=>$project_id,'givenprojectname'=>$given_project_name,'po_number'=>$startreorder,'van_stock'=>0,'Transaction_id'=>'none','payment_status'=>'none','reorder'=>1);	
						...
						$message='<html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8" /><style>td {vertical-align: -webkit-baseline-middle;}</style></head>
								  ...
								  <td align="center">
									...
									<table width="600px" border="0" cellspacing="0" cellpadding="0" align="center">
											<tr>
													<td  width="400px;"><h3 style="font-weight: bold;">PURCHASE REORDER</h3>
