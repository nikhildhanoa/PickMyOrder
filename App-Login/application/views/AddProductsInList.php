...
error_reporting(0); ?>
 <script src="https://cdn.tiny.cloud/1/grycy333jpmalqgmpv4daavj5ie8s20cscvmx2ecr1ax0f7g/tinymce/5/tinymce.min.js" referrerpolicy="origin"></script>
...
<?php $checkcat=$this->ProductCategoryModal->GetAllCat($bid); if($checkcat){ ?>
  <ul class="nav nav-tabs">
    <li class="<?php echo (isset($tab)) ? ($tab['sub']=="pd")? 'active' : '' : ''; ?>"><a class="active" data-toggle="tab" href="#pd" style="padding: 10px 13px;">Product Details</a></li>
    ...
    <li class="disablealert disabled<?php //echo (isset($tab)) ? ($tab['sub']=="sc")? 'active' : '' : ''; ?>"><a data-toggle="tab" href="#sc"  style="padding: 10px 13px;">Suppliers Cost</a></li>
     ...
     $role=$_SESSION['status']['role'];
	   ...
	   <li class="li-file"><form id="sendcsvtoanotherfile" enctype="multipart/form-data" method="post" action="uploadcsvfilebyajax"><label  style="padding: 9px 7px;">Lukins import<input type="file" accept=".csv" id="importcsv" size="60" name="thisiscsv"></label></form></li>
	   
	     <li style="margin: 0 9px;" class="li-file"><form id="sendcsvtoanotherfilebulk" enctype="multipart/form-data" method="post" action="uploadcsvfilebyajaxbulk"><label style="padding: 9px 13px;">Bulks import<input type="file" accept=".csv" id="importcsvbulk" size="60" name="thisiscsv"></label></form></li>
		 
		 
<!-- <li style="margin-right: 9px;" class="li-file"><form id="sendupdatecsv" enctype="multipart/form-data" method="post" action="UpdateProductsWithCsvFile"><label style="padding: 9px 7px; cursor: default;">update<input type="file" accept=".csv" id="updatecsvjax" size="60" name="thisiscsv"></label></form></li>-->
		  
		
		  
	   <li class="li-file firstli"><span class="luckinimport btn btn-info" style="padding: 8px 7px;font-weight:700;    cursor: default;">Lukins</span><li>
  ...
  <!--<ul style="display:flex;justify-content:flex-end;list-style:none;position: relative;top: -20px;right: 136px;">
      <li style="padding: 0 37px;"><span style="font-weight:bold">Lukins</span></li>
      <li style="padding: 0 19px;"><span style="font-weight:bold;position: relative;left: -8px;">Bulks</span></li>
  </ul>-->
 <!-----------------------------------------------Modal popup----------------------------------------->
...
<?php } ?>
 ...
 <div class="form-group ">
      ...
      <textarea type="text" id="single_decsription" class="form-control form-control-solid newtextarea"  name="Description"><?= isset($_POST['Description'])? $_POST['Description']: '';?></textarea>
...
<?php    $supplierobj=$this->db->query("select suppliers_name,id from dev_supplier where business_id='$bu_id'");
					  $supplierarray=$supplierobj->result_array(); 
					  ?>
 ...
 <?php foreach($AllCat as $All){ $cat_name=$All['cat_name']; $Cat_Cute_Name=$All['Cat_Cute_Name'];   $name = (empty($Cat_Cute_Name)) ? $cat_name : $Cat_Cute_Name;  ?>
 <option  <?php if(isset($_POST['Category'])){ if($_POST['Category']==$All['id']){?> selected <?php }  } ?> value=<?php echo $All['id']; ?>><?php   echo $name; ?></option>
 <?php } ?>
 </select>
  ...
  <select class="form-control form-control-solid"  name="Sub_Sub_Category" id="subsubcat">
   <option value="" selected disabled>--Select--</option>
	  ...
	  <div class="form-group">
      <label id="supersubcat">Sub_Sub_Sub_Category</label>
