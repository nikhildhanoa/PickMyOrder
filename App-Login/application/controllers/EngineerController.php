	     ...
	     if($this->input->post('add_enginner'))
			{
				...
				$brands=$this->db->query("select brandpage,iswholeapp,iswholpageview from dev_business where id=$business_id ");
			    $results=$brands->result_array();
		        $brands_pre = $results[0]['brandpage'];  
				$iswholeapp = $results[0]['iswholeapp'];
				$iswholpageview = $results[0]['iswholpageview'];
			 ...
			 // if($this->input->post('vanstock')){$eng_p13='1';}else{$eng_p13='0';}
			   if($this->input->post('basic_User')){$eng_p14='1';}else{$eng_p14='0';}
		...
		/***Permission*****/
		 ...
		 $sopd=$this->input->post('single_order_per_day');
				...
				$sopd='100000';
				 ...
				 $Eng_permi_data=array('Supervisor'=>$eng_p1,'Operative'=>$eng_p2,'All_orders'=>$eng_p3,'Orders_to_Approve'=>$eng_p4,'See_costs'=>$eng_p5,'limit_order_per_day'=>$opd,'single_order_limit'=>$sopd,'Categories'=>$eng_p6,'Orders'=>$eng_p7,'Project_details'=>$eng_p8,'Catalogues'=>$eng_p10,'AddProduct'=>$eng_p11,'Quotes'=>$eng_p12,'is_basic_user'=>$eng_p14,'card'=>$cardpremission);
