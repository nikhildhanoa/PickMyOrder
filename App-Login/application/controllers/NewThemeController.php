...
class NewThemeController extends CI_Controller
{
	...
	Public function SendDataInPmoSupplierPageFromXero()
	{
	...
	try {
		  $getContactss = $apiInstance->getContacts($xeroTenantId,$order);
				   ...
				   $contact_HTML  .='<option value="'.$get_Single_Contacts['name'].'" >'.$get_Single_Contacts['name'].'</option>';  
					  }
					...
					$item_HTML  .='<option value="'.$get_single_item['name'].'" >'.$get_single_item['name'].'</option>';
		          ...
		          return  $pass_array=array('contact'=>$contact_HTML,'category'=>$item_HTML);
	...
	public function getNewSuppliersPage()
	{
		 ...
		 $Get_xero_data = $this->SendDataInPmoSupplierPageFromXero();
			...
			$this->load->view('AddNewSupplier.php',$Get_xero_data);
...
$All_opprative= $this->CommonModel->Fetchjoindata($select,$TableName,$opprative_multiClause,$result_as_array,$join_table,$join_relation);
		    ...
		    $this->load->view('header.php');
			$this->load->view('sidebar.php');
			$this->load->view('AddNewProjects.php',$engineer_Send_in_view);
			$this->load->view('footer.php');
			 }
	}
   ...
   /******************Print pdf Table ***************************/
	...
	/******************************printwholesellertable****************************/
		...
		public function GetAllNewProductList()
		{
		   ...
		   $AllCatListData= $this->db->query("select * from dev_productlist order by choice DESC");
		...
		{
		 $id=$_REQUEST['id'];
		 $select='dev_orders.*,dev_projects.invioce_Order_Number ';
		 $TableName='dev_orders';
		 $multiClause=array('dev_orders.givenprojectid'=>$id);
		 $join_table='dev_projects';
		 $join_relation='dev_projects.id=dev_orders.givenprojectid';
		 $result_as_array=true;
		$OrderUnderProject= $this->CommonModel->Fetchjoindata($select,$TableName,$multiClause,$result_as_array,$join_table,$join_relation);	
		echo json_encode($OrderUnderProject); die;
		}
		/****************************************************Awting Order View****************************/
