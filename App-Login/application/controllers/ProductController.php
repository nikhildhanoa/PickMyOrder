...
else
  {
	...
	public function Products(){
		...
		if(isset($_SESSION['status'])){
			...
			if($this->input->post('sub_Sub_Sub_Category'))
			{
				$super_sub_sub_cat_id=$this->input->post('sub_Sub_Sub_Category');
				//print_r($super_sub_sub_cat_id);die;
				$getsupersubsubcatname=$this->db->query("select name from dev_super_sub_sub_cat where id='$super_sub_sub_cat_id'");
				$getsupersubsubcatname=$getsupersubsubcatname->result_array();
				$supersubsubcatnameis=$getsupersubsubcatname[0]['name'];
				//print_r($supersubsubcatnameis);die;
			}
			else
			{
				$super_sub_sub_cat_id=0;
				$supersubsubcatnameis='';
			}
			...
			'ex_vat'=>$exvat,'tax_class'=>$this->input->post('tax'),'publish_status'=>$publish,
			'categories'=>$CAT_ID,'sub_categories'=>$SUB_CAT_ID,'super_sub_cat_id'=>$SUPER_SUB_CAT_ID,
			'searchsupercat'=>$supersubcatnameis,'searchsubcat'=>$subcatnameis,'searchcat'=>$catnameis,'AddedFilters'=>serialize($FillterArrayEmpty),'dateandtime'=>$date,'user_id'=>$MainUserId,'super_sub_sub_cat_id'=>$super_sub_sub_cat_id,'searchsupercat'=>$supersubcatnameis);
              ...
              $table='dev_products';				
				...
				if($insert)
				{
				   $this->session->set_flashdata('successdetails', "Product Details Add Successfully");
					...
					$err='';
					if($invan==1 && $this->input->post('pub')==1 )
					{	
						$err='&error=van';
					}
				
					/********************close***********************************/
					redirect('Edit_single_product?id='.$Last_Insert_Id.$err);
				}
				else
				{
...
public function AddVanStock(){
   ...
   $vanupdate=$this->db->insert('products_van', $data);
	if($vanupdate)
	{
		echo "1";
	}
...
public function UpdateProducts()
{
	...
	{  $FillterArrayEmpty=array();
		...
		if($this->input->post('Sub_Sub_Category'))
			...
			if($this->input->post('sub_Sub_Sub_Category'))
			{
				$sub_Sub_Sub_Category_id=$this->input->post('sub_Sub_Sub_Category');
				
				$getsupersubsubcatname=$this->db->query("select name from dev_super_sub_sub_cat where id='$sub_Sub_Sub_Category_id'");
				$getsupersubsubcatname=$getsupersubsubcatname->result_array();
				if($getsupersubsubcatname)
				{
				$superssububcatnameis=$getsupersubsubcatname[0]['name'];
				}
				else
				{
					$superssububcatnameis='0';
				}
			}
			else
			{
				$sub_Sub_Sub_Category_id=0;
				$superssububcatnameis='';
			}
		 ...
		 if($this->input->post('filteritem') && $this->input->post('filteritemvalue'))
			...
			/* if($this->input->post('invan'))
			{
			    $invan=1;
				if($this->input->post('pub')==1)
				{
					
					$checksupplier=$this->db->query("select supplier_with_cost from  dev_products where id='$product_hidden_id'");
					 $checksupplier=$checksupplier->result_array();
					 if($checksupplier[0]['supplier_with_cost']=='')
					 {
						echo "Operation Failed !!! Van products cannot published without choosing a supplier";
						die;
					 }		
                ...
                $stocklevel=$this->input->post('stocklevel');
		...
		/*  if($InStock>$reorderlevel){
			...
			$this->db->query("update dev_products set toback_instock= '' where id=$product_hidden_id"); 
		 } */
		 //else
		 //{
			/*  $_SESSION['tab']='';
	...
	if(isset($_FILES['product_image_1']) || isset($_FILES['product_image_2']) )  //update images
	{
		 ...
		 $product_image_2=$_FILES['product_image_2'];
			...
			/* 
			if($product_image_3['size']!=0)
			{
				$image_name_3=$product_image_3['name'];
				 $uniqueid=rand();
				 $image_name_3=$uniqueid.$image_name_3;
				 $image_name_3=str_replace(' ', '',$image_name_3);
				 $url_image_name_3=$this->UpdateUrlMethod().'/images/products/'.$image_name_3;
				  $data=array('product_image_three'=>$url_image_name_3);
		           $where=array('id'=>$product_hidden_id);
				 if(move_uploaded_file($product_image_3["tmp_name"],"images/products/".$image_name_3)){
					//$this->DataModel->update($table,$data,$where);
					$this->db->query("update dev_products  set product_image_three = '$url_image_name_3' where id='$product_hidden_id'");
				 }
	...
	if(isset($_POST['hideen_pid_supplier']))  //update suppliers cost
	{
		...
		$product_id=$_POST['hideen_pid_supplier'];
		 ...
		 for($i=0;$i<=$numbersupp;$i++){
			 if($this->input->post('supp'.$i)&&$this->input->post('suppcost'.$i)){
				...
				if($i==0)
				{
					$mainsupplier=$_POST['supp0'];
				}
				if($this->input->post('suppliercost')){
					
					$supplier_select_box=$_POST['suppliercost'];
					$mainsupplier=$_POST[$supplier_select_box];
				}
		...
		$supplier_with_cost=rtrim($suppliercost,",");
		 ...
		 $this->db->query("update dev_products SET supplier_with_cost='$supplier_with_cost',main_supplier_id='$mainsupplier' where id='$product_id'");
...
public function uploadcsvfilebyajax() // Full bulk upload not for luckin only
{
	$flag='0';
	...
	{   
	          ...
	          $bid=$res['Business ID'];
			 if( $res['Category ID']!='' && $res['Sub Category ID']!=''  && $res['Super Sub Cat ID']!=''   && $res['Product Name']!='' && $res['Title']!='' && $res['Description']!=''  && $res['Specification']!=''  && $res['Vat Deductable']!='' && $res['Ex_vat']!='' && $res['Publish_Status']!='' && $res['Products_image_one']!='' && $res['PDF1']!='' && $res['Business ID']!='' && $res['UniqueCode']!='')
				{
					$empty_status="1";
				...
				else
				{
				 ...
				 if($res['Tsicode']!='')
				 {
					if( $res['Category ID']!='' && $res['Sub Category ID']!=''  && $res['Super Sub Cat ID']!=''  &&$res['Business ID']!=''  && $res['Product Name']!=''  && $res['Vat Deductable']!='' && $res['Ex_vat']!='' && $res['UniqueCode']!='' )
					{
						$empty_status_tsi="1";
						
					} 
					
					if( is_numeric($res['Category ID']) && is_numeric($res['Sub Category ID']) && is_numeric($res['Super Sub Cat ID'])  && is_numeric($res['Business ID']) &&  is_numeric($res['Ex_vat']) && is_numeric($res['Vat Deductable']) )
					{
				...
				/*********************************************TSI Close*************************************/
		    if(is_numeric($res['List']) && is_numeric($res['Category ID']) && is_numeric($res['Sub Category ID']) && is_numeric($res['Super Sub Cat ID']) &&  is_numeric($res['Ex_vat'])  && is_numeric($res['Business ID'])  && is_numeric($res['Vat Deductable']))
				{
					$numeric_status="1";
				}
				else
				...
				/*  echo  $empty_status;
				echo $numeric_status;
				echo $exsist_bussiness;
				
				echo $exsist_supersubcat;
				echo $exsist_subcat;
				echo  $exsist_cat;
				
				echo $status_vat_deduct;
				echo $numeric_status_tsi;
				echo $empty_status_tsi;
				echo $exsist_list;
				exit;  */
			 ...
			 if($empty_status=='1' && $numeric_status=='1' && $exsist_bussiness=='1' && $exsist_supersubcat=='1' && $exsist_subcat=='1' &&  $exsist_cat=='1' || $status_vat_deduct=='1' && $numeric_status_tsi=="1" && $empty_status_tsi=="1" && $exsist_list=="1")
			{
				...
				if(is_numeric($res['tax class']) && ($res['tax class']==5 || $res['tax class']==10 || $res['tax class']==20 || $res['tax class']==0))
				{
					$res['Inc_vat']=$res['Ex_vat']+$res['Ex_vat']*$res['tax class']/100;
					
						
				}
				else
				{
					$res['Inc_vat']=$res['Ex_vat']+$res['Ex_vat']*20/100;
					
				}
				$res['Inc_vat']=number_format($res['Inc_vat'],2);
 ...
 $CsvlineNumber++;   
     ...
     if($flag=='0')
	 {
	   ...
	   /****************************insert variation ***********************************/
         ...
         echo "<a href='AddNeThemeProduct'>BACK</a>";	 
	 }
	 else
	 {
...
/*******************************************close********************************************/	
	  public function GenrateTokenForLuckin()   //Token genrate
	  {
		  $curl = curl_init();
	         ...
	         return    $mytokenfromr=rtrim($mytokenfromltrim, '"');
				 
				 if($mytokenfromr!='')
				 {
					  $time=date("h:i:sa");
					 
					 $Token_array=array('token'=>$mytokenfromr,'Token_Time'=>$time);
					 $token_id=1;
					 $this->db->where('id', $token_id);
                     $this->db->update('dev_lukins_token', $Token_array);
	...
	public function FetchingDataFromLuckins()   //function for feaching data from lucins api
	{
	   if(isset($_SESSION['Current_Business']))
		...
		$myflag=0;
		  ...
		  //SELECT token FROM dev_lukins_token ORDER BY id DESC LIMIT 1;
        ...
        $luckingData=$this->db->query("select Luckins  from dev_business where id='$bussiness_id'");
		$data2=$luckingData->result_array();
		$get_lucking_info=$data2[0]['Luckins'];
		...
		{    
			...
			CURLOPT_POSTFIELDS => " {\r\n\"AssetSized2List\":[{\r\n\"Height\":400,\r\n\"MaintainAspectRatio\":true,\r\n\"Tag\":\"\",\r\n\"Width\":323\r\n}],\r\n\"TSIUniqueIdentifierPairList\":[{\r\n\"TSIUniqueIdentifierType\":1,\r\n\"TSIUniqueIdentifier\":\"$tsicode\"\r\n}],\r\n\"Token\":\"$token\",\r\n\"ViewportType\":3\r\n}\r\n\r\n",
				...
				else
				{
					...
					$productData=$this->db->query("select tax_class,vat_deductable  from dev_products where id='$productid'");
					$productData=$productData->result_array();
					
					$tax_class=$productData[0]['tax_class'];	
					$vat_deductable=$productData[0]['vat_deductable'];	
						...
						if($get_lucking_info=="1")
						{
							...
							/* $this->db->query("update dev_products set pdf_name='$luckname1' ,pdf2_name='$luckname2' ,title='$title',description='$description',products_images='$image1',product_image_two='$image2',product_image_three='$image3',product_image_four='$image4',product_image_five='$image5',pdf_manual='$pdf1',pdf_manual2='$pdf2',specification='$specification',product_name='$Shortkey',SKU='$sku',inc_vat='$Inc_vat',ex_vat='$BasePrice',lukin_update='1' where id=$productid"); */
						...
						if($this->db->affected_rows())
						{
							$myflag=$myflag+1;
								  ...
								  //$update_Failure_array=array('lukin_error_info'=>'ALL');			
   ...
   /*****************************************************************************************/
	...
	public function copypmoproductsinproductsList()
	{
		...
		if(empty($listarray))
		{
			$newencoded=json_encode(array($Listid));
			$this->db->query("update dev_products SET list_id='".$newencoded."' where id='$productid'");
		}
		else{
	...
	public function RemoveListFromAProduct()
	{
		...
		if(in_array($Listid,$listarray))
		{
			...
			$newencoded=json_encode($listarray);
		    $this->db->query("update dev_products SET list_id='".$newencoded."'where id='$productid'");
		    redirect(base_url('ManageListProducts/'.$Listid));
		}
	}	
	...
	public function ApplyCatFillterOnListProduct()
	{
		...
		$proidarray=array();
		$query="select dev_products.id id,title,products_images,SKU,publish_status from dev_product_cat,dev_products where  dev_product_cat.id=categories and list_id like'%".'"'.$Listid.'"'."%'"; 
		if($catid!=0)
		{
			
		$query=$query." and categories=$catid";
		
		}
		if($manufactures!="0")
		{
			$query=$query." and  Manufacture='$manufactures'";
             			
		}
	
			$listobj=$this->db->query($query);
	...
	public function Copy_List_Product()
	{
		...
		{   
			...
			$all_proids=implode(',',$proidarray);
			
			$listobj=$this->db->query("select id,product_name,title,categories,sub_categories,super_sub_cat_id from dev_products where id in($all_proids)");
		...
		else
		{
				...
				{
					$listobj=$this->db->query("select id,product_name,title,categories,sub_categories,super_sub_cat_id,super_sub_sub_cat_id from dev_products where business_id=$source_business");
				...
				{	
					...
					$listobj=$this->db->query("select id,product_name,title,categories,sub_categories,super_sub_cat_id,super_sub_sub_cat_id from dev_products where list_id like'%".'"'.$listid.'"'."%'");
		...
		{    
			...
			if($list['categories']!='' && $list['categories']!=0)
			{
				$cat=$list['categories'];
				$CatData=$this->db->query("select * from dev_product_cat where id=$cat");
				$CatData=$CatData->result_array();
				$cat_name=$CatData[0]['cat_name'];
				$cat_image=$CatData[0]['image'];
				
				
			}
			if($list['sub_categories']!='' && $list['sub_categories']!=0)
			{
				$sub_cat=$list['sub_categories'];
				$subCatData=$this->db->query("select * from dev_product_sub_cat where id=$sub_cat");
				$subCatData=$subCatData->result_array();
				$sub_cat_name=$subCatData[0]['sub_cat_name'];
				$sub_cat_image=$subCatData[0]['image'];
			}
			if($list['super_sub_cat_id']!='' && $list['super_sub_cat_id']!=0)
			{
				$super_sub_cat=$list['super_sub_cat_id'];
				$supersubCatData=$this->db->query("select * from super_sub_cat where id=$super_sub_cat");
				$supersubCatData=$supersubCatData->result_array();
				$super_sub_cat_name=$supersubCatData[0]['name'];
				$super_sub_cat_image=$supersubCatData[0]['image'];
			}
			if($list['super_sub_sub_cat_id']!='' && $list['super_sub_sub_cat_id']!=0)
			{
				$super_sub_sub_cat=$list['super_sub_sub_cat_id'];
				$supersubCatData=$this->db->query("select * from dev_super_sub_sub_cat where id=$super_sub_sub_cat");
				$supersubCatData=$supersubCatData->result_array();
				$super_sub_sub_cat_name=$supersubCatData[0]['name'];
				$super_sub_sub_cat_image=$supersubCatData[0]['image'];
			}
			...
			$this->db->where('business_id',$destination_business);
			$this->db->like('title',$title,'none');
		
			// $this->db->or_like(array('product_name' => $name, 'title' => $title),'none');
			...
			if($product->num_rows())
			{
				...
				$getCatDataName='';
				$getsubCatDataName='';
				$getsupersubCatDataName='';
				$getsupersubsubCatDataName='';
				...
				foreach($product as $productdata)
				{
					...
					if($productdata['super_sub_sub_cat_id']!='' && $productdata['super_sub_sub_cat_id']!=0)
					{
						$getsupersubsubCatData=$this->db->query("select * from dev_super_sub_sub_cat where id=".$productdata['super_sub_sub_cat_id']);
						$getsupersubsubCatData=$getsupersubsubCatData->result_array();
						$getsupersubsubCatDataName=$getsupersubsubCatData[0]['name'];
						
					}
					if($productdata['super_sub_cat_id']!='' && $productdata['super_sub_cat_id']!=0)
					{
						$getsupersubCatData=$this->db->query("select * from super_sub_cat where id=".$productdata['super_sub_cat_id']);
						$getsupersubCatData=$getsupersubCatData->result_array();
						$getsupersubCatDataName=$getsupersubCatData[0]['name'];
						
					}
					if($productdata['sub_categories']!='' && $productdata['sub_categories']!=0)
					{
						$getsubCatData=$this->db->query("select * from dev_product_sub_cat where id=".$productdata['sub_categories']);
						$getsubCatData=$getsubCatData->result_array();
						$getsubCatDataName=$getsubCatData[0]['sub_cat_name'];
						
					}
					if($productdata['categories']!='' && $productdata['categories']!=0)
					{
						$getCatData=$this->db->query("select * from dev_product_cat where id=".$productdata['categories']);
						$getCatData=$getCatData->result_array();
						$getCatDataName=$getCatData[0]['cat_name'];
					...
					elseif(strtolower($cat_name)==strtolower($getCatDataName) )
					{
						...
						elseif(strtolower($sub_cat_name)==strtolower($getsubCatDataName))
						{
							$match=1;
							
							
							if($super_sub_cat_name=='')
							{
								
								$match=1;
								break;
							}
							elseif(strtolower($super_sub_cat_name)==strtolower($getsupersubCatDataName))
							{
								$match=1;
								
								
								if($super_sub_sub_cat_name =='')
								{
									$match=1;
									break;
								}
								elseif(strtolower($getsupersubsubCatDataName)==strtolower(getsupersubsubCatData))
								{
									$match=1;
									break;
								}
			...
			{	
					...
					if($cat_name!=='')
					{
						...
						$this->db->where('business_id',$destination_business);
						$this->db->like('cat_name',$cat_name,'none');
						...
						if($cat_data->num_rows())
						{
							...
							if($sub_cat_name!=='')
							{
								...
								$this->db->like('sub_cat_name',$sub_cat_name,'none');
								$sub_cat_data=$this->db->get();
								
								if($sub_cat_data->num_rows())
								{
									$sub_cat_data=$sub_cat_data->result_array();
									$sub_cat_id=$sub_cat_data[0]['id'];
									...
									if($super_sub_cat_name!=='')
									{
										...
										$this->db->like('name',$super_sub_cat_name,'none');
										$super_sub_cat_data=$this->db->get();
										
										if($super_sub_cat_data->num_rows())
										{
											 $super_sub_cat_data=$super_sub_cat_data->result_array();
											 $super_sub_cat_id=$super_sub_cat_data[0]['id'];
											...
											if($super_sub_sub_cat_name!=='')
											 {
													...
													$this->db->where('super_sub_cat',$super_sub_cat_id);
													$this->db->like('name',$super_sub_sub_cat_name,'none');
												$super_sub_sub_cat_data=$this->db->get();
												
												if($super_sub_sub_cat_data->num_rows())
										       {
												 $super_sub_sub_cat_data=$super_sub_sub_cat_data->result_array();
											     $super_sub_sub_cat_id=$super_sub_sub_cat_data[0]['id'];
											   ...
											   else
											   {
												  if($super_sub_sub_cat_name!='')
												   {
													   $supersubsubCatArr=array('super_sub_cat'=>$super_sub_cat_id,'name'=> $super_sub_sub_cat_name,'image'=>$super_sub_sub_cat_image);
												      $Insert=$this->db->insert('dev_super_sub_sub_cat',$supersubsubCatArr);
													  $super_sub_sub_cat_id=$this->db->insert_id();
										...
										else
										{
											...
											if($super_sub_cat_name!='')
											{
												$superCatArr=array('sub_cat'=>$sub_cat_id,'name'=> $super_sub_cat_name,'image'=>$super_sub_cat_image);
												$Insert=$this->db->insert('super_sub_cat',$superCatArr);
												$super_sub_cat_id=$this->db->insert_id();
								...
								else
								{
									...
									$subcatarr=array('cat_id'=>$cat_id,'sub_cat_name'=>$sub_cat_name,'image'=>$sub_cat_image);
						...
						else
						{
							...
							$CatArr=array('userid'=>$user_id,'business_id'=>$destination_business,'cat_name'=>$cat_name,'image'=>$cat_image);
						 ...
						 $status=$this->db->query("INSERT INTO `dev_products`(  `Manufacture`,`super_sub_sub_cat_id`, `super_sub_cat_id`, `sub_categories`, `categories`, `searchcat`,
							 ...
							 `pdf2_name`, `pdf3_name`, `pdf4_name`, `pdf5_name`, `pdf6_name`, `supplier_with_cost`, `dateandtime`,lukin_update) SELECT  
							 `Manufacture`,$super_sub_sub_cat_id,$super_sub_cat_id, $sub_cat_id, $cat_id, `searchcat`, `searchsubcat`, `searchsupercat`,
							 ...
							 `pdf6_name`, `supplier_with_cost`, now(),lukin_update FROM `dev_products` WHERE id=$productid");
							 if($status)
							 {
					...
					else
					{
						 $status=$this->db->query("INSERT INTO `dev_products`(  `Manufacture`, `super_sub_cat_id`, `sub_categories`, `categories`, `searchcat`,
							 ...
							 `product_image_two`, `pdf_manual`, `pdf_manual2`, `pdf_manual3`, `pdf_manual4`, `pdf_manual5`, `pdf_manual6`, `pdf_name`, 
							 `pdf2_name`, `pdf3_name`, `pdf4_name`, `pdf5_name`, `pdf6_name`, `supplier_with_cost`, `dateandtime`,lukin_update) SELECT  
							 `Manufacture`,0, 0, 0, `searchcat`, `searchsubcat`, `searchsupercat`,
							 `Tsicode`, `product_name`, `title`, `description`, `reviews`, `specification`, `price`, `inc_vat`,
							 `ex_vat`, `vat_deductable`, `publish_status`, `tax_class`, `products_images`, `product_image_two`, `pdf_manual`, `pdf_manual2`,
							 `pdf_manual3`, `pdf_manual4`, `pdf_manual5`, `pdf_manual6`, `pdf_name`, `pdf2_name`, `pdf3_name`, `pdf4_name`, `pdf5_name`,
							 `pdf6_name`, `supplier_with_cost`, now(),lukin_update FROM `dev_products` WHERE id=$productid");
...
public function uploadluckincsvfilebyajax() // Luckins only upload......
{
	$flag='0';
	 ...
	 if(isset($_SESSION['Current_Business']))
			{
			  ...
			  $luckingData=$this->db->query("select Luckins  from dev_business where id='$bussiness_id'");
			  $data=$luckingData->result_array();
			  $get_lucking_info=$data[0]['Luckins'];  
...
/*********** Inc_vat set in lucking== 1 *************/			 
			 ...
			 if( $res['Category ID']!='' && $res['Sub Category ID']!=''  && $res['Super Sub Cat ID']!='' && 
			 $res['Vat Deductable']!='' && $res['Ex_vat']!='' && $res['Publish_Status']!=''  && $res['Business ID']!='' && $res['UniqueCode']!='' && $res['Tsicode']!='')
				{
					$empty_status="1";
			 ...
			 if($empty_status=='1' && $numeric_status=='1' && $exsist_bussiness=='1' && $exsist_supersubcat=='1' && $exsist_subcat=='1' &&  $exsist_cat=='1' || $status_vat_deduct=='1' && $numeric_status_tsi=="1" && $empty_status_tsi=="1" && $exsist_list=="1")
			{
				...
				if($res['Title']=='')
				{
					$res['Title']=$res['Tsicode'];
				}
				
				
				if($res['tax class'] && ($res['tax class']==5 || $res['tax class']==10 || $res['tax class']==20 || $res['tax class']==0))
				{
					$res['Inc_vat']=$res['Ex_vat']+$res['Ex_vat']*$res['tax class']/100;
 ...
 $CsvlineNumber++;   
   ...
   /*************************/
    ...
    public function AddCategoryInListCategory()
		{
            ...
            redirect(base_url('index'));
			  ...
			  $category_List_id=$_GET['listid'];
			  $data_send=array();
			
			$cat_id=$this->db->query("select Category_ids from  dev_categorylist where id='$category_List_id'");
			...
			//print_r($Category_ids);die;
               ...
               if($Category_ids!='')
			   {				   
			 $cat_data=$this->db->query("select  id,cat_name,image from   dev_product_cat where id IN ($Category_ids)");
			 $cat_data=$cat_data->result_array(); 
			
			$data_send=array('key'=>$cat_data);
			
			   }else
			   {
				    $data['error']="<p style='color:red;text-align:center;font-weight:bold'>Invalid userid or password !</p>";
			...
			// $this->load->view('AddCategoryInListCategory.php',$data_send);
			  
			  
			  
			  $this->load->view('header.php');
			  $this->load->view('sidebar.php');
			  $this->load->view('AddCategoryInListCategory.php',$data_send, $data);
			  $this->load->view('footer.php');
		...
		/***********AddCategoryInCategoryList**********/
		 
		 public function AddCategoryInCategoryList()
		 {
			 $blank_array= array();
			 $val=$_POST['val'];
			 $cat_List_Id=$_POST['catListId'];
			
			foreach($val as $statusids)
			{	
                 $statu_array= array('cat_ids'=> $statusids);
				 array_push($blank_array,$statu_array);
			}
			 ...
			 $Category_ids=$cat_id[0]['Category_ids'];
				 ...
				 if($Category_ids!=='')
				 {
						$cat_ids=$cat_ids.','.$Category_ids;
				 }
					 
				 
				// print_r($cat_List_Id);die;
				
					$data_array=array('Category_ids'=>$cat_ids);
					$this->db->where('id', $cat_List_Id);
					$this->db->insert_batch('dev_categorylist_status',$blank_array);
					$insert_data= $this->db->update('dev_categorylist', $data_array);
		...
		public function DeleteCategoryForCategorylist()
		{
			$cat_id=$_GET['id']; //
			$cat_list_id=$_GET['listid'];
			
			$this->db->query("delete from  dev_categorylist_status  where cat_ids='$cat_id' ");
			 $cat_ids=$this->db->query("select Category_ids from  dev_categorylist where id='$cat_list_id' ");
			$cat_ids=$cat_ids->result_array();
			$Category_ids=$cat_ids[0]['Category_ids'];
			
           $ids_array=explode(",",$Category_ids);			///
			$key = array_search($cat_id,$ids_array);
			
			unset($ids_array[$key]);
			
			  $imload_ids=implode(',',$ids_array);
		     // 
			  
			$data_array=array('Category_ids'=>$imload_ids);
			
			$this->db->where('id',$cat_list_id);
			$update_data= $this->db->update('dev_categorylist',$data_array);
		
			  if($update_data)
			 {
				 redirect('AddCategoryInListCategory?listid='.$cat_list_id);
			 } 
		 ...
		 public function CopyCategoryInAnotherList()
		 {
			 $categories_id=$_POST['val']; // array of new cats to copied
			 $catListid=$_POST['catListId'];
			 
			 $imload_ids=implode(',',$categories_id);
			 
			
			$cat_ids=$this->db->query("select Category_ids from  dev_categorylist where id='$catListid' ");
			$cat_ids=$cat_ids->result_array();
			$Category_ids=$cat_ids[0]['Category_ids']; // already exists cat id in cat list
			$ids_array=explode(",",$Category_ids);
			...
			$update_data= $this->db->update('dev_categorylist',$data_array);
             ...
             else
			 {
				...
				{		
					...
					$ids_array[]=$categories_id[$i];
					
				}
				
				
				$result = array_unique($ids_array);
				$Category_ids=implode(',',$result);
				
				  $data_array=array('Category_ids'=>$Category_ids);
	 ...
	 public function LukinsFailureProducts()
	 {
		  if(empty($this->session->userdata('status'))){
            redirect(base_url('index'));
			 }
	        ...
	        $this->load->view('header.php');
			$this->load->view('sidebar.php');
			$this->load->view('LukinsFailureProducts.php');
			$this->load->view('footer.php'); 
			 }
     }
	...
	public function GetLukinsFailureProducts()
	{
	...
	if(isset($_SESSION['Current_Business']))
		   {
			   ...
			   $this->db->select('*');
                $this->db->from('dev_products');
				$array = array('business_id' => $bid, 'lukin_update' =>2);
                 $this->db->where($array);
				 $query = $this->db->get();
                  $Failure_product = $query->result_array();
		...
		echo json_encode($Failure_product);
		
	}


       public function LuckinsProducts()
	 {
		  if(empty($this->session->userdata('status'))){
            redirect(base_url('index'));
			 }
	        ...
	        $this->load->view('header.php');
			$this->load->view('sidebar.php');
			$this->load->view('LuckinsProducts.php');
			$this->load->view('footer.php'); 
			 }
     }
	 ...
	 {
		  $this->load->view('login-2.php');
	 }

     public function InsertLukinsTempData()
	 {
		   
		 $key = array();
		 $qry = $this->db->query("select COM_Code from dev_temp_ITEMCODE");
		 $qrydata=$qry->result_array();
		 
		
		$token=$this->GenrateTokenForLuckin();
		 
		  foreach($qrydata as $tsicode)
		  {
		  
		   $tsicode =  $tsicode['COM_Code'];
		 ...
		 $curl = curl_init();
			...
			CURLOPT_POSTFIELDS => " {\r\n\"AssetSized2List\":[{\r\n\"Height\":400,\r\n\"MaintainAspectRatio\":true,\r\n\"Tag\":\"\",\r\n\"Width\":323\r\n}],\r\n\"TSIUniqueIdentifierPairList\":[{\r\n\"TSIUniqueIdentifierType\":1,\r\n\"TSIUniqueIdentifier\":\"$tsicode\"\r\n}],\r\n\"Token\":\"$token\",\r\n\"ViewportType\":3\r\n}\r\n\r\n",
				...
				echo '<pre>';
				die; */
				     $ResultCount  = $myJSON->ResultCount;
				   ...
				   $data =array('pdf_name'=>$luckname1,'pdf2_name'=>$luckname2,'products_images'=>$image1,'product_image_two'=>$image2,'product_image_three'=>$image3,'pdf_manual'=>$pdf1,'pdf_manual2'=>$pdf2);
				  // print_r($data);die;
				   array_push($key,$data);
				   
		           }
				   
				  $insert = $this->db->insert_batch('dev_temp_lukins_data', $key); 
				   if($insert)
				   {
					   echo "insert";
				   }else{"not";}
          ...
          public function ManageListProducts()
	        {
			if(empty($this->session->userdata('status'))){
				redirect(base_url('index'));
				 }
				 else
				 {
					...
					$this->load->view('header.php');
					$this->load->view('sidebar.php');
					$this->load->view('ManageListProducts.php');
					$this->load->view('footer.php'); 
				 }
	   }
	   ...
	   public function GetAllListProducts()
		...
		if($null!='null')
		{
		  ...
		  {
			   $this->db->where("".$Like." LIKE '%".$by1."%' ");
		  }
		  
		  if($subsubcat_id!='0')
		  {
			  $this->db->where(array('categories'=>$cat_id,'sub_categories'=>$subcat_id,'super_sub_cat_id'=>$subsubcat_id)); 
		  }
          elseif($subcat_id!='0')
		  {
			$this->db->where(array('categories'=>$cat_id,'sub_categories'=>$subcat_id));   
		  }
          elseif($cat_id!='0')
          {
			$this->db->where(array('categories'=>$cat_id));   
		  }		  
		  
		  $single_Product_index = $this->db->get()->result_array();
		  /* print_r($this->db->last_query());die; */
		}	
		else{
		
		$SingleListObject=$this->db->query("select * from dev_products where list_id like '%".'"'.$listid.'"'."%'");
	     $single_Product_index=$SingleListObject->result_array();
		}
		
		print_r(json_encode($single_Product_index));
		}
		
		 public function AddProductsInList()
	        {
			if(empty($this->session->userdata('status'))){
				redirect(base_url('index'));
				 }
				 else
				 {
					...
					$this->load->view('header.php');
					$this->load->view('sidebar.php');
					$this->load->view('AddProductsInList.php');
					$this->load->view('footer.php'); 
				 }
	   }
